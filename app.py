...
def upload_dataset():
    ...
    data_file = st.file_uploader(
        "To begin, please upload your prepared dataset.", type=['xlsx', 'xls'])
...
def validate_data(data):
    ...
    with data_preview:
        sheet_name = st.selectbox(
            "Select sheet to preview.", list(data.keys()))
    ...
    if 'grades' in sheet_types:
        grade_validation = st.expander(
            "Validate academic score labels", expanded=False)
        with grade_validation:
            # present validation options
            st.markdown(
                'The following sheetnames were detected to contain academic scores. \nPlease confirm the extracted metadata about each set of task scores:')
            task_names = []
            for sheet_name in sheet_types['grades']:
                students_to_annotate.append(
                    data[sheet_name][['Student Id', 'Surname', 'First Name']])
                st.write(f'**{sheet_name}**')
                col1, col2, col3 = st.columns(3)
                _, year = [x.strip('') for x in sheet_name.split(' ')]
                year = col1.text_input(f"Year", year, key=f'{sheet_name}_year')
                task_cols = [col for col in data[sheet_name].keys()
                             if 'task' in col.lower()]
                for task in task_cols:
                    task_number = col2.text_input(f"Task number", task.split(' ')[
                                                  1], key=f'{year}_{task}_number')
                    task_maximum = col3.text_input(
                        f"Task maximum", task.split('/')[-1], key=f'{year}_{task}_max')
                    data[sheet_name].rename(
                        columns={task: f'Task {task_number} total / {task_maximum}'}, inplace=True)
                    task_names.append(
                        [f'Grades {year}', year, f'Task {task_number} total / {task_maximum}', task_number, task_maximum])
                data[f'Grades {year}'] = data.pop(sheet_name)
            task_names = pd.DataFrame(task_names, columns=[
                                      'Sheet name', 'Year', 'Column name', 'Task number', 'Maximum score'])
        # compile students from all task sheets for annotation
        students_to_annotate = pd.concat(
            students_to_annotate).drop_duplicates()

        # -----check for student info, assigned tracked students-----
        Gender_assignment = st.expander(
            "Assign student identifiers", expanded=False)
        with Gender_assignment:
            # prepare data
            students_to_annotate['Full Name'] = students_to_annotate['First Name'] + \
                ' ' + students_to_annotate['Surname']
            Genders = {}
            ...
            if 'Students to Track' in sheet_names:
                data['Students to Track']['Full Name'] = data['Students to Track']['First Name'] + \
                    ' ' + data['Students to Track']['Surname']
                for student in data['Students to Track']['Student Id'].tolist():
                    students_to_track.append(student)

            # Collect annotations
            st.markdown(
                'Please confirm all student Gender annotations, and select which students to track using the left checkbox')
            col1, col2, col3 = st.columns([2, 0.5, 0.5])
            for index, student in students_to_annotate.drop_duplicates().iterrows():
                student_id = student['Student Id']
                if student_id in students_to_track:
                    tracked = col1.checkbox(
                        f"{student['Full Name']}", value=True, key=f'{student_id}_track')
                else:
                    tracked = col1.checkbox(
                        f"{student['Full Name']}", value=False, key=f'{student_id}_track')
            ...
            students_to_annotate['Gender'] = students_to_annotate['Student Id'].map(
                Genders)
            students_to_annotate['tracked'] = students_to_annotate['Student Id'].map(
                trackers)

            data['Student Info'] = students_to_annotate

    if 'wellbeing' in sheet_types:
        wellbeing_validation = st.expander(
            "Validate wellbeing labels", expanded=False)
        with wellbeing_validation:
            st.markdown(
                'The following sheetnames were detected to contain wellbeing scores:')
    ...
    summary = st.expander("Dataset Summary & Download", expanded=True)
    with summary:
        if 'unknown' in sheet_types:
            st.markdown(
                '**The following sheetnames were not recognised and were not be processed**:')
            st.write(f"*{', '.join(sheet_types['unknown'])}*")

        # preview student info
        st.markdown(
            f"**Total number of students**: {len(data['Student Info'])}")
        st.markdown(
            f"**Number of tracked students**: {len(data['Student Info'][data['Student Info']['tracked'] == 1])}")
        st.markdown(
            f"**Students with unknown gender**: {len(data['Student Info'][data['Student Info']['Gender'].isnull()])}")
        ...
        if col1.button("Download compiled dataset"):
            tmp_download_link = download_link(df_to_excel_bytes(
                data), 'compiled_data.xlsx', 'Click here to download!')
            col1.markdown(tmp_download_link, unsafe_allow_html=True)

        if col2.button('Download Student Information'):
            tmp_download_link = download_link(
                data['Student Info'], 'student_info.csv', 'Click here to download!')
...
def summarise_dataset(data):
    ...
    for sheet_name, dataframe in task_names.groupby(['Sheet name']):
        ...
        for col_name, df in dataframe.groupby(['Column name']):
            if df['Maximum score'].tolist()[0] != 100:
                year_data[col_name] = year_data[col_name] / \
                    df['Maximum score'].tolist()[0] * 100
            year_data[col_name] = year_data[col_name].astype(float)
            year_data.rename(
                columns={col_name: f"Task {df['Task number'].tolist()[0]}"}, inplace=True)
    ...
    tasks['Gender'] = tasks['Student Id'].map(
        dict(student_info[['Student Id', 'Gender']].values))
    tasks['Tracked'] = tasks['Student Id'].map(
        dict(student_info[['Student Id', 'tracked']].values))
    ...
    wellbeing['Full Name'] = wellbeing['First Name'] + \
        ' ' + wellbeing['Surname']

    type_labels = {0: 'Whole School',
                   1: 'Positives English', 2: 'Negatives English'}
    wellbeing['type_order'] = wellbeing['type'].map(
        {v: k for k, v in type_labels.items()})

    type_keys = [tuple(key) for key in wellbeing[['year', 'type_order']].sort_values(
        ['type_order', 'year'], ascending=True).drop_duplicates().values.tolist()]
    type_order = dict(zip(type_keys, np.arange(0, len(type_keys))))

    term_keys = [tuple(key) for key in wellbeing[['year', 'Term']].sort_values(
        ['year', 'Term'], ascending=True).drop_duplicates().values.tolist()]
    term_order = dict(zip(term_keys, np.arange(0, len(term_keys))))

    wellbeing['type_position'] = [
        tuple(x) for x in wellbeing[['year', 'type_order']].values]
    wellbeing['type_position'] = wellbeing['type_position'].map(type_order)
    wellbeing['term_order'] = [tuple(x)
                               for x in wellbeing[['year', 'Term']].values]
...
def zip_download(memory_zip, filename):
    ...
    tmp_download_link = download_link(
        data, f'{filename}.zip', 'Click here to download!')
...
def generate_introduction():
    ...
    template = pd.read_excel('utilities/template.xlsx',
                             sheet_name=None, engine='openpyxl')
    example = pd.read_excel('utilities/example_data.xlsx',
                            sheet_name=None, engine='openpyxl')

    template_link = download_link(df_to_excel_bytes(
        template), 'template.xlsx', 'template document')
    example_link = download_link(df_to_excel_bytes(
        template), 'example_data.xlsx', 'example dataset')
    ...
    process_data_instructions = st.expander(
        "Data preprocessing", expanded=False)
    ...
    analysis_instructions = st.expander("Analysis", expanded=False)
    ...
    download_instructions = st.expander(
        "Download summary graphs", expanded=False)
...
def main():
    ...
    st.set_page_config(page_title='STUDENT PERFORMANCE ANALYSER AND REPORT GENERATOR', page_icon=favicon,
    ...
    app_mode = st.sidebar.selectbox("Choose functionality:",
                                    ["Home", "Prepare dataset", "Academic performance analysis", "Wellbeing analysis"])
    ...
    if app_mode == 'Prepare dataset':
        ...
        if st.button('Submit'):
            ...
            state.data = data
            st.subheader(
                "Congratulations! Your data has been compiled. You may now proceed to the analysis panes using the navigation bar to the left.")
        ...
        if state.data_processed:
            ...
            st.sidebar.markdown(
                "To begin, you must validate your prepared dataset. \n Please select 'Prepare data'.")
...
def run_grades():
    ...
    task_keys = [tuple(key) for key in cohort[['Year', 'Task']].sort_values(
        ['Year', 'Task'], ascending=True).drop_duplicates().values.tolist()]
    task_order = dict(zip(task_keys, np.arange(0, len(task_keys))))

    cohort['task_order'] = [tuple(key)
                            for key in cohort[['Year', 'Task']].values]
    ...
    students_to_track = dict(
        class_students[['Full Name', 'Student Id']].values)
    ...
    col1, col2 = st.columns(2)
    col1.markdown(
        "This graph displays the average task score for all tracked students (line) +/- the [standard deviation](https://en.wikipedia.org/wiki/Standard_deviation) (band).")
    class_color = col1.color_picker(
        'To select a new class color:', value='#800080')
    ...
    col1, col2 = st.columns(2)
    col1.markdown(
        "This graph displays the average task score for all tracked students compared to the entire cohort.")
    cohort_color = col1.color_picker(
        'To select a new cohort color:', value='#ff8c00')

    fig = plot_class_v_cohort_grades(
        cohort, task_order, untracked_color=cohort_color, tracked_color=class_color)
    col2.pyplot(fig)
    ...
    col1, col2 = st.columns(2)
    col1.markdown(
        "This graph displays the average task score for males versus females from the entire cohort.")
    male_color = col1.color_picker(
        'To select a new male color:', value='#0652ff')
    female_color = col1.color_picker(
        'To select a new female color:', value='#fe01b1')

    fig = plot_gender_grades(
        cohort, task_order, female_color=female_color, male_color=male_color)
    ...
    st.markdown(
        "This graph displays an individual student's score compared to the male and class averages.")
    student = st.selectbox(
        'Please select a student to preview:', list(students_to_track.keys()))

    col1, col2, col3 = st.columns([3, 3, 1])
    student_color = col3.color_picker(
        'To select a new student color:', value='#601ef9')
    ...
    if len(student_df) < 1:
        ...
        fig = plot_student_vs_collection_grades(student_df, student_label=name, collection_df=male_cohort,
                                                collection_label='Average Male', task_order=task_order, collection_color=cohort_color, student_color=student_color)
        col1.pyplot(fig)
        plt.close(fig)

        fig = plot_student_vs_collection_grades(student_df, student_label=name, collection_df=class_students,
                                                collection_label='Class Average', task_order=task_order, collection_color=class_color, student_color=student_color)
        col2.pyplot(fig)
        plt.close(fig)
    ...
    if col1.button('Generate all individual student plots'):
        for student in students_to_track.values():
            ...
            fig = plot_student_vs_collection_grades(student_df, student_label=name, collection_df=male_cohort,
                                                    collection_label='Average Male', task_order=task_order, collection_color=cohort_color, student_color='#601ef9')
            grades_figures[f'{name}_v_males'] = fig
            plt.close(fig)

            fig = plot_student_vs_collection_grades(student_df, student_label=name, collection_df=class_students,
                                                    collection_label='Class Average', task_order=task_order, collection_color=class_color, student_color='#601ef9')
            grades_figures[f'{name}_v_class'] = fig
            plt.close(fig)
        state.grades_figures = grades_figures
    if hasattr(state, 'grades_figures'):
        col2.markdown(
            f'{int((len(state.grades_figures.keys()) - 3) / 2)} student plots successfully generated.')
        if col1.button('Download plots'):
            zf, memory_zip = zip_images(state.grades_figures)
            tmp_download_link = zip_download(
                memory_zip, filename='academic_scores')
            col2.markdown(tmp_download_link, unsafe_allow_html=True)
...
def run_wellbeing():
    ...
    source = wellbeing.copy().groupby(
        ['year', 'Term', 'type', 'type_position']).sum()['count'].reset_index()
    source = pd.pivot_table(source, values='count', columns=[
                            'type_position'], index='Term')

    st.subheader("Class Summary")
    st.markdown(
        "This graph displays the sum of all incidences across each year, broken down by term.")
    ...
    student = st.selectbox('Please select a student to preview:',
                           wellbeing['Full Name'].unique().tolist())
    ...
    palette = {'Whole School': ws_color,
               'Positives English': pos_color, 'Negatives English': neg_color}
    ...
    if len(student_df) < 1:
        ...
        sns.lineplot(data=student_df, x='term_order', y='count',
                     hue='type', palette=palette, linewidth=5)
    ...
    if col1.button('Generate all individual student plots'):
        for student, df in wellbeing.groupby('Full Name'):
            fig, ax = plt.subplots(figsize=(10, 5))
            sns.lineplot(data=df, x='term_order', y='count',
                         hue='type', palette=palette, linewidth=5)
            ha.make_wellbeing_student_pretty(ax, term_order, student)
    ...
    if hasattr(state, 'wellbeing_figures'):
        col2.markdown(
            f"{len(state.wellbeing_figures.keys()) - 1} student plots successfully generated.")
        if col1.button('Download plots'):
...
def plot_student_vs_collection_grades(student_df, student_label, collection_df, collection_label, task_order, collection_color='purple', student_color='#601ef9'):
    ...
    fig, ax = plt.subplots()
    sns.lineplot(data=collection_df, x='task_order', y='score', label=collection_label,
                 ci='sd', color=collection_color, alpha=0.5, marker='o')
...
def plot_class_v_cohort_grades(cohort, task_order, untracked_color='darkorange', tracked_color='purple'):
    ...
    fig, ax = plt.subplots()
    sns.lineplot(data=cohort, x='task_order', y='score', hue='Tracked', hue_order=[
                 0, 1], palette=[untracked_color, tracked_color], ax=ax, ci='sd', marker='o')
...
def plot_class_grades(cohort, task_order, class_color='purple'):
    ...
    fig, ax = plt.subplots()
    # plot whole class
    sns.lineplot(data=cohort[cohort['Tracked'] == 1], x='task_order', y='score',
                 label='Class Average', color=class_color, alpha=0.5, ci='sd', marker='o')
    ax.set_xlabel('')
    ha.make_grades_pretty(ax, task_order)
    plt.tight_layout()
